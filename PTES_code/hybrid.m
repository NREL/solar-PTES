...
% Declare fluids and specify initial conditions for packed bed regenerator
        ...
        F1.state(iL,i1).T = 581; %in Kelvin
        F1.state(iL,i1).mdot = 78;
        
        % Nitrogen (cold, low pressure)
        F2 = fluid_class('Nitrogen','WF','CP','TTSE',1,5);
        F2.state(iL,i2).p = 10e5;
        F2.state(iL,i2).T = 450;
        F2.state(iL,i2).mdot = 78;
...
k_gas_hot = 45e-3; %thermal conductivity of nitrogen
...
mu_g_cold = 24*10^-6;
%Cp_cold=1040;
Cp_cold = 1049; %Cold Nitrogen specific heat
%k_gas_cold =0.028;
k_gas_cold=29e-3;
%rho_g_cold = 11.23;
...
dp = 0.02; %Diameter of the particle in m
rho_particle = 2700; %Density of the particle
phi = 0.41; %Porosity of bed assumed
D = 1.5; %Diameter of the bed in m


%Packed bed Regenerator assumptions
Ph=40; % Assumed Hot cycle duration (in seconds)
Pc=40; % Assumed Cold cycle duration
...
epsilon = 0.93; %From lookup table
...
PB=PB_class(L, D, Ph, Pc, Nu_PB, Nu_PB2, C_me, NTUe, eff_re, delP_hot,delP_cold, TC);
PB.Length = L;
PB.Diameter = D;
PB.hot_time=Ph;
PB.cold_time=Pc;
...
PB.total_cost=TC;
 ...
 fprintf('Length  Diameter  hot_time  cold_time  Nu_hotstream  Nu_coldstream  CM  NTU_hotstream  eff  PB.pressure_drop_hot  PB.pressure_drop_cold  PB.total_cost\n')
 fprintf('%6.2f %7.2f %8.2f %12.2f %11.3f %12.3f %9.2f %10.3f %8.2f %16.4f %18.4f %19.1e \n',...
     PB.Length,PB.Diameter,PB.hot_time,PB.cold_time,PB.Nu_hotstream,PB.Nu_coldstream,PB.CM,PB.NTU_hotstream,PB.eff, PB.pressure_drop_hot/1e5, PB.pressure_drop_cold/1e5, PB.total_cost);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        F1.state(iL,i1).T = 450; %in Kelvin
        F1.state(iL,i1).mdot = 78;
