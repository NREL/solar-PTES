...
Tp0   = Tsat - 25 ; % Initial temperature of PCM
xp0   = 0.0 ;
...
dTm   = 5.0 ;           % Melting temperature range of PCM
...
%hs(:,:)   = [linspace(hv,hl,Nx)' linspace(hv,hl,Nx)'] ;
%xs(:,:)   = [linspace(1.0,0.0,Nx)' linspace(1.0,0.0,Nx)'] ;
%for i = 1 : Nx
%    rhos(i,1) = RP1('PQ_INPUTS',Ps0,xs(i,1),'D',steam) ; 
%    rhos(i,2) = rhos(i,1) ;
%end
