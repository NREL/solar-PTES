...
PRr     = 0.95;          % discharge pressure ratio: PRdis = PRch*PRr
...
if Ncld == 1
    ...
    for ii = 1 : Ncld
        fluidC(ii)  = fluid_class(char(fCname(ii,:)),'SF','TAB',NaN,Load.num,2);
...
% Hot tanks
if Nhot == 1
    fluidH(1:Nc_ch) = fluid_class(fHname,'SF','TAB',NaN,Load.num,2); % Storage fluid
    HT  = double_tank_class(fluidH,TH_dis0,p0,MH_dis0,TH_chg0,p0,MH_chg0,T0,Load.num+1); %hot double tank
else
    for ii = 1 : Nhot
        fluidH(ii)  = fluid_class(char(fHname(ii,:)),'SF','TAB',NaN,Load.num,2);
