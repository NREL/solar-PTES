...
stages_chg = find(~strcmp({gas.stage(ip1,:).type},'0'),1,'last')-1;
stages_dis = find(~strcmp({gas.stage(ip2,:).type},'0'),1,'last')-1;
...
if WM==1
    ...
    for i0=1:stages_chg+1, fprintf(1,'%11.1f %11.1f %11.2f %11.2f %11.1f %11s %11s\n', gas.state(ip1,i0).T, gas.state(ip1,i0).p/1e5, gas.state(ip1,i0).h/1e6, gas.state(ip1,i0).s/1e3, gas.state(ip1,i0).mdot, gas.stage(ip1,i0).type, 'ch');end
    fprintf(1,'\n');
    for i0=1:stages_dis+1, fprintf(1,'%11.1f %11.1f %11.2f %11.2f %11.1f %11s %11s\n', gas.state(ip2,i0).T, gas.state(ip2,i0).p/1e5, gas.state(ip2,i0).h/1e6, gas.state(ip2,i0).s/1e3, gas.state(ip2,i0).mdot, gas.stage(ip2,i0).type, 'dis');end
    fprintf(1,'\nHot fluid streams:\n');
    fprintf(1,'-->%s\n',fluidH(1).name);
    fprintf(1,'%11s ','Tin[K]','Tout[K]','Δh [MJ/kg]','Δs [kJ/kg/K]','mdot[kg/s]','Stream','Cycle'); fprintf(1,'\n');
    for i0=1:nH, fprintf(1,'%11.1f %11.1f %11.2f %11.2f %11.2f %11d %11s\n', fluidH(i0).state(ip1,1).T, fluidH(i0).state(ip1,2).T, (fluidH(i0).state(ip1,2).h - fluidH(i0).state(ip1,1).h)/1e6, (fluidH(i0).state(ip1,2).s - fluidH(i0).state(ip1,1).s)/1e3, fluidH(i0).state(ip1,1).mdot, i0, 'ch');end
    for i0=1:nH, fprintf(1,'%11.1f %11.1f %11.2f %11.2f %11.2f %11d %11s\n', fluidH(i0).state(ip2,1).T, fluidH(i0).state(ip2,2).T, (fluidH(i0).state(ip2,2).h - fluidH(i0).state(ip2,1).h)/1e6, (fluidH(i0).state(ip2,2).s - fluidH(i0).state(ip2,1).s)/1e3, fluidH(i0).state(ip2,1).mdot, i0, 'dis');end
    fprintf(1,'\nCold fluid streams:\n');
    fprintf(1,'-->%s\n',fluidC(1).name);
    fprintf(1,'%11s ','Tin[K]','Tout[K]','Δh [MJ/kg]','Δs [kJ/kg/K]','mdot[kg/s]','Stream','Cycle'); fprintf(1,'\n');
    for i0=1:nC, fprintf(1,'%11.1f %11.1f %11.2f %11.2f %11.2f %11d %11s\n', fluidC(i0).state(ip1,1).T, fluidC(i0).state(ip1,2).T, (fluidC(i0).state(ip1,2).h - fluidC(i0).state(ip1,1).h)/1e6, (fluidC(i0).state(ip1,2).s - fluidC(i0).state(ip1,1).s)/1e3, fluidC(i0).state(ip1,1).mdot, i0, 'ch');end
    for i0=1:nC, fprintf(1,'%11.1f %11.1f %11.2f %11.2f %11.2f %11d %11s\n', fluidC(i0).state(ip2,1).T, fluidC(i0).state(ip2,2).T, (fluidC(i0).state(ip2,2).h - fluidC(i0).state(ip2,1).h)/1e6, (fluidC(i0).state(ip2,2).s - fluidC(i0).state(ip2,1).s)/1e3, fluidC(i0).state(ip2,1).mdot, i0, 'dis');end
    fprintf(1,'\nHot tanks\n');
...
switch mode
    ...
    case 2 % Heat engine only
        ...
        rhoE = W_out_dis/fact/(HT.A(4).V)*1e3; %kWh/m3
        %rhoP_dis = (W_out_dis/t_dis/(gas_min_rho_dis.mdot/gas_min_rho_dis.rho)/1e6); %MW/(m3/s)
end
