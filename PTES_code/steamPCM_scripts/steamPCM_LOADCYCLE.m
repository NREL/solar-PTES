...
if strcmp(Load(Iload),'c')
    ...
    if Lreadload
        mdot(Iload) = dsg_mdot(Iload) - dsg_mdot0 ;
        ...
        hs    = RP1('PQ_INPUTS',PsC,xs(1),'H',steam) * ones(Nx,2) ;
        ss    = RP1('PQ_INPUTS',PsC,xs(1),'S',steam) * ones(Nx,2) ;
        rhos  = RP1('PQ_INPUTS',PsC,xs(1),'D',steam) * ones(Nx,2) ;
        mus   = RP1('PQ_INPUTS',PsC,xs(1),'VISCOSITY',steam) * ones(Nx,2) ;
        us    = Gs ./ rhos ;
    else
...
elseif strcmp(Load(Iload),'d')
    if Lreadload
        mdot(Iload) = dsg_mdot0 - dsg_mdot(Iload) ; % Probably need something more sophisticated than this based on power
        xs    = dsg_q0 * ones(Nx,2) ; % Dryness fraction
        Gs    = mdot(Iload)  * ones(Nx,2) / Ap ; % Mass flux
        Ts    = TsD  * ones(Nx,2) ; % Temperature
        Ps    = PsD   * ones(Nx,2) ; % Pressure
        hs    = RP1('PQ_INPUTS',PsD,xs(1),'H',steam) * ones(Nx,2) ;
        ss    = RP1('PQ_INPUTS',PsD,xs(1),'S',steam) * ones(Nx,2) ;
        rhos  = RP1('PQ_INPUTS',PsD,xs(1),'D',steam) * ones(Nx,2) ;
        mus   = RP1('PQ_INPUTS',PsD,xs(1),'VISCOSITY',steam) * ones(Nx,2) ;
        us    = Gs ./ rhos ;
    else
