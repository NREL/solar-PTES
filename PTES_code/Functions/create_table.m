...
elseif cond2
    ...
    switch Mname
        ...
        case 'AlkaneMix' % Mixture from Kalina paper
            Tbot  = 5  + 273.15;
            Ttop  = 60 + 273.15;
...
elseif cond2 % Extract data from CoolProp (requires access to  Python interface)
    ...
    switch Mname
        ...
        case 'AlkaneMix'
            % Data from REFPROP
            T_dat   = [5.0, 11.1, 17.2, 23.3, 29.4, 35.6, 41.7, 47.8, 53.9, 60.0] + 273.15;
            rho_dat = [564.3, 556.8, 549.1, 541.2, 533.0, 524.5, 515.7, 506.4, 496.7, 486.4];
            k_dat   = [0.111, 0.108, 0.105, 0.103, 0.100, 0.097, 0.094, 0.092, 0.089, 0.087];
            Pr_dat  = [3.32, 3.26, 3.21, 3.17, 3.12, 3.08, 3.04, 3.01, 2.98, 2.95];
            mu_dat  = [1.541, 1.453, 1.371, 1.293, 1.219, 1.149, 1.083, 1.019, 0.957, 0.897]*1e-4;
            %Cp_dat  = [2396, 2432, 2470, 2511, 2555, 2603, 2655, 2712, 2777, 2851];
            Cp_dat  = Pr_dat.*k_dat./mu_dat;
